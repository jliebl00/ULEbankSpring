...
<body>
	<header>
		...
		<a class="optionsHeader assets" href="<c:url value="assets.htm"/>"
			...
			class="optionsHeader payments" href="<c:url value="payments.htm"/>"
			title="Payments"></a>
	</header>
	...
	<div class="content">
		<div class="headerContent">
			<b class="titleContent"><fmt:message key="offices" /></b> <a
				class="searcherContent" href="<c:url value="searcher.htm"/>"
				title="Search office"></a>
		</div>

		<ul>
			<c:forEach items="${model.offices}" var="offic">

				<li><form name="form" method="GET"
						action="liabilitiesoffice.htm">

						<input type="hidden" name="officeID" value="${offic.officeID}">

						<b>Id: </b>
						<c:out value="${offic.officeID}" />

						<i><b>Address: </b> <c:out value="${offic.address}" /></i> <input
							type="submit" name="submit" value="Access">
